...
router.post('/answer/:questionId', async (ctx) => {
    ...
    let response = ctx.request.body;
    let {boxId, questionEmail} = (await ctx.db.query('SELECT box_id,email FROM questions WHERE _id = $1;', [questionId])).rows[0];
    await verifyPasswordFromHeader(ctx, boxId);
    response = createResponse(response);
    ...
    if(questionEmail)
        ctx.notification.notifyAnswer(questionEmail, response._id).catch(e => console.error(e));
    ctx.response.status = 201;
})
